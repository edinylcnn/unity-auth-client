...
namespace Auth
{
    ...
    {
        [Header("Config")]
        public AppConfig config; // Inspector’dan AppConfig.asset bağla

        string BaseUrl => config != null ? config.baseUrl : "http://localhost:5000";

        static string ExtractMessage(string json)
        {
            try { return JsonUtility.FromJson<ErrorMsg>(json).message; }
            catch { return json; }
        }
        ...
        [Serializable] class LoginRequest  { public string UsernameOrEmail; public string Password; }

        public IEnumerator SignUp(string username, string email, string password, Action<bool,string> cb)
        {
            var body = JsonUtility.ToJson(new SignUpRequest { Username=username, Email=email, Password=password });
            ...
            if (req.result != UnityWebRequest.Result.Success) { cb(false, req.error); }
        ...
        public IEnumerator GetMe(string token, Action<bool,UserMe,string> cb)
        {
            ...
            if (req.result != UnityWebRequest.Result.Success) { cb(false, null, req.error); }
